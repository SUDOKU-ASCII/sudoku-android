...
- Global network settings card (shared by all nodes): routing mode (Global/Direct/PAC), PAC rule URLs, and DNS/IP preference (default / IPv4-only / IPv6-preferred).
...
2. Run `scripts/build_sudoku_aar.sh`, which clones upstream `sudoku` at `SUDOKU_REF` (default: `main`, i.e. upstream v0.3.0), executes `gomobile bind` (default targets: `android/arm,android/arm64`) on `./pkg/mobile`, and drops the AAR into `app/libs/`.
